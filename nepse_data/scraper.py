...
DATA_DIR = Path(__file__).parent / "data"  # canonical, CWD-independent
...
class BaseScraper(ABC):
    ...
    def run(self, dates: list[date]) -> dict[date, int]:
        ...
        for idx, d in enumerate(dates, 1):
            ...
            if df is not None and not df.empty:
                ...
                self.log.warning(f"  ↳ no data returned for {d} (holiday / no trading?)")
            if idx < total:
    ...
    @abstractmethod
    def _scrape(self, d: date) -> pd.DataFrame | None:
    ...
    def _scrape(self, d: date) -> pd.DataFrame | None:
        ...
        try:
            ...
            while True:
                count += 1
                if count > MAX_PAGES:
                    self.log.warning(f"Aborting pagination for {d}: exceeded {MAX_PAGES} pages")
                    break
...
class MerolaganiScraper(BaseScraper):
    ...
    def _parse_table(self, html: str) -> pd.DataFrame:
        ...
        table = (
            soup.find("table", {"id": "ctl00_ContentPlaceHolder1_StockTradeVolumeTable"})
...
def main() -> None:
    ...
    dates = [date.today()] if args.today else trading_days_in_range(args.start, args.end)
    if not dates:
    ...
    log.info(
        f"Scraping {len(sources)} source(s) × {len(dates)} date(s). Output → {DATA_DIR.resolve()}"
    )
